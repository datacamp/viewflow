...
from . import dependencies_patterns
...
def get_rmd_dependencies(rmd_content, schema_name, task_name, custom_function: str) -> List[str]:
    if custom_function:
        return getattr(dependencies_patterns, custom_function)(rmd_content, schema_name, task_name)
    return getattr(dependencies_patterns, "get_dependencies_default")(rmd_content, schema_name, task_name)
    

def get_r_dependencies(r_content, schema_name, custom_function: str) -> List[str]:
    if custom_function:
        return getattr(dependencies_patterns, custom_function)(r_content, schema_name)
    return getattr(dependencies_patterns, "get_dependencies_default")(r_content, schema_name)
